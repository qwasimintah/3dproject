...
class VertexArray:
    def __init__(self, attributes, index=None):
        ...
        # if index != None:
        self.buffers += [GL.glGenBuffers(1)]
        # print (self.buffers[len(attributes)])
...
def main():
    ...
    # limb_shape = Node(transform=translate(0, 2, 0) @ rotate(axis=(1, 0, 1), angle=120, radians=None) @ scale(1, 1, 1))  # make a thin cylinder
    # limb_shape.add(cylinder)          # common shape of arm and forearm

    # arm_node = Node(transform=translate(0, 2, 0) @ rotate(axis=(1, 0, 1), angle=20, radians=None) @ scale(.4, 1, .4), children=[cylinder]) # robot arm rotation with phi angle
    # arm_node.add(limb_shape)

    # base_shape = Node(transform=translate(0, 0, 0) @ rotate(axis=(0,0, 0), angle=0, radians=None) @ scale(1, 1, 1), children=[cylinder])
    # base_node = Node(transform=translate(0, -0.2, 0) @ rotate(axis=(0, 0, 0), angle=0, radians=None) @ scale(.5, .7, .5))   # robot base rotation with theta angle
    # base_node.add(base_shape, arm_node)

    # viewer.add(base_node)
    
    # vector_keyframes = KeyFrames({0: 1, 3: 7, 6: 20})
    # x = vector_keyframes.value(1.5)
    # print (x)
    # print(vector_keyframes.value(26))   # should display numpy vector (0.5, 0.5, 0)

    # Viewerr.add(cylinder_node)
    viewer.run()
