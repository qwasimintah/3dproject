...
class VertexArrayInst:
    ...
    def draw(self, primitive):
        """draw a vertex array, either as direct array or indexed array"""
        GL.glBindVertexArray(self.glid)
        self.draw_command(primitive, *self.arguments, 10)
        GL.glBindVertexArray(0)
...
void main() {
    ...
    gl_Position = modelviewprojection * (rotation * vec4(position ,1) + vec4(offset,1));
...
class TexturedMesh:
    ...
    def draw(self, projection, view, model, win=None, **_kwargs):
        ...
        translations = np.zeros(shape=(10, 3))
        ...
        for i in range(10):
            translations[i][0] += y * sin(i/10)
            translations[i][1] += -sin(i/2)
        ...
        for i in range(10):
            ...
            l = GL.glGetUniformLocation(self.shader.glid, 'rotations[%d]' % i)
            #GL.glUniform3fv(l, 1, rotations[i])
            GL.glUniformMatrix4fv(l, 1, True, rotate(axis=vec(0,1,0),angle=alpha))
...
class TexturedPlane1:
    ...
    def draw(self, projection, view, model, win=None, **_kwargs):
        ...
        for i in range(PARTICLES):
            ...
            y = g[1]*tfasti*tfasti + (5.5*((i*i)/2) *tfasti)
...
def main():
    ...
    #viewer.add(*tree)
    viewer.add(TexturedPlane1('water.jpeg'))
