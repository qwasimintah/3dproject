...
# class Particle:
#     def__init__(self, pos, vel, col, ang):
#         self.postion = pos
#         self.speed = vel
#         self.color = col
#         self.life = 100
...
#         self.offsets = []
#         self.particlesSystem = ParticlesSystem(number)
...
#         for  x in range(0, self.particleSystem.number):

#             attributes.append()

#         self.vertex_array = VertexArrayInst(attributes, index)
...
TEXTURE_VERT_INSTS = """#version 330 core
...
TEXTURE_FRAG_INSTS = """#version 330 core
out vec4 outColor;
uniform sampler2D diffuseMap;
in vec2 fragTexCoord;
void main() {

    float r = texture(diffuseMap, fragTexCoord)[0]; 
    float g = texture(diffuseMap, fragTexCoord)[1]; 
    float b = texture(diffuseMap, fragTexCoord)[2]; 
...
class ParticleMesh:
    ...
    def __init__(self, texture, nb_instances, attributes, index):
        ...
        self.texture = texture
        self.positions = None
        self.speed=[vertices]*nb_instances
        self.delta=0.0005
    def draw(self, projection, view, model, win=None, **_kwargs):
        ...
        # offset the instance
        for i, pos in enumerate(self.positions):
            self.speed[i] += vec(0.001,0.001, 0.0001) * self.delta * 0.005* i;
            #self.speed[i] += vec(0.1,0.1, 0.1) *  0.05;
            loc_offset = GL.glGetUniformLocation(self.shader.glid, 'offsets[%d]' % i)
            GL.glUniform3fv(loc_offset, 1, self.speed[i])
...
class TexturedMeshInstNormal:
    """ Simple first textured object """

    def __init__(self, texture, nb_instances, attributes, index):
        # feel free to move this up in the viewer as per other practicals
        self.shader = Shader(TEXTURE_VERT_INSTS, TEXTURE_FRAG_INSTS)

        # triangle and face buffers
        vertices = attributes[0]
        self.tex_uv = attributes[1]
        #self.normals = (attributes[2]) # why put them in self?
        self.vertex_array = VertexArrayInst(nb_instances, [vertices, self.tex_uv], index)
...
def load_textured_n_instances(file, nb_instances):
    ...
    for mesh in scene.meshes:
        ...
        meshes.append(ParticleMesh(texture, nb_instances, [mesh.vertices, tex_uv, mesh.normals], mesh.faces))
...
def load_textured_n_instances2(file, nb_instances):
    """ load resources using pyassimp, return list of TexturedMeshes """
    try:
        option = pyassimp.postprocess.aiProcessPreset_TargetRealtime_MaxQuality
        scene = pyassimp.load(file, option)
    ...
    for mesh in scene.meshes:
        ...
        # create the textured mesh object from texture, attributes, and indices
        meshes.append(TexturedMeshInstNormal(texture, nb_instances, [mesh.vertices, tex_uv], mesh.faces))
...
def main():
    ...
    #trees = load_textured_n_instances2("Flower/Flower.obj",len(tree_bases))
    #trees = texture_lib.load_textured("Flower/Flower.obj")
    # viewer.add(trees)
    # print(len(trees))
    for t in trees:
    ...
    viewer.add(TexturedMesh(Texture('grassfl.png'), [vertices, tex_uv, normals], faces))
