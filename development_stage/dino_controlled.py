...
ANGLE_INCR = 1
sinalpha = [sin(i*ANGLE_INCR * pi/180) for i in range(int(360/ANGLE_INCR) + 1)]
cosalpha = [cos(i*ANGLE_INCR * pi/180) for i in range(int(360/ANGLE_INCR) + 1)]
    ...
    def run(self):
        ...
        while not glfw.window_should_close(self.win):
            ...
            for x in range(20):
                ...
                if zt > -camera_posisition[1]:
                    # we're under the surface - that's not what we want
                    z = self.campos[1]
                    camera_posisition = self.pos + self.campos * (27-x)/27.0
                    self.campos[1] = z
                else:
                    break
            view = rotate(axis=vec(0,1,0), angle=self.alpha) @ translate(*camera_posisition)
            # delme
            self.view = view
    ...
    def on_key(self, _win, key, _scancode, action, _mods):
        ...
        if action == glfw.PRESS or action == glfw.REPEAT:
            if key == glfw.KEY_T:
                ...
                print(self.view)
                print("# d.transform : ")
                print(self.controlled.node.transform)
                print("# self.alpha :")
                print(self.alpha)
                print("# self.pos :")
                print(self.pos)
            ...
            if key == glfw.KEY_A:
                ...
                #d.transform = translate(*pos) @ rotate(axis=vec(0,1,0), angle=ANGLE_INCR) @ translate(*(-1*pos)) @ d.world_transform
                ...
                self.campos = self.camera + (self.dist * sin(alpha), 0, self.dist * (1-cos(alpha)) )
                #self.campos = self.camera + (self.dist * sinalpha[self.alpha], 0, self.dist * (1-cosalpha[self.alpha]) )
